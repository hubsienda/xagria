...
export default function DeskShell({children}: {children: React.ReactNode}) {
  return <div lang="en" className="mx-auto flex min-h-screen max-w-6xl flex-col px-4 sm:px-6 lg:px-8">
    <header className="flex flex-wrap items-center justify-between gap-4 py-6">
      ...
      <span className="text-sm text-muted">Agricultural tools & intelligence</span>
    </header>
    <main className="flex-1 py-10">{children}</main>
    <footer className="border-t border-white/10 py-6 text-sm text-muted">XAGRIA · Sienda Ltd</footer>
  </div>;
}
